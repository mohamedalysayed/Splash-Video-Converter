...
<p align="center">
  <img width="900" alt="Cast converting a queue of files" src="https://github.com/user-attachments/assets/71edc6cf-dfe7-4e21-81b3-ffd07680f934" />
</p>
...
So I wrote Cast. One window. One queue. The conversion happens and it's beautiful while it does. That's the whole thing.
...
| OS | File | Notes |
|---|---|---|
| **macOS** (Apple Silicon) | `Cast-macos-arm64.dmg` | macOS 11+. M1/M2/M3/M4. Intel returns in v1.1. |
...
Apple charges $99/year to notarize apps. Until Cast makes enough to justify it, macOS will refuse the app on first launch with *"Cast is damaged and can't be opened."* One-time fix in Terminal:

```bash
xattr -cr /Applications/Cast.app
```
...
**FFmpeg**: if it's already on your `PATH`, Cast finds it. If not, the welcome screen offers a one-click download into Cast's private folder. No admin rights, no PATH editing.
...
1. **Drop files** — single clips, whole folders, mixed types. Cast recursively finds every video and audio file inside.
...
You can cancel at any time. Partial output files get cleaned up automatically. Your source files are never touched — if an output name would collide, Cast auto-numbers (`clip (2).mp4`).
...
**Outputs** are opinionated — Cast picks the codec combination that works:
...
# → dist/Cast       (Linux)
# → dist/Cast.exe   (Windows)
# → dist/Cast.app   (macOS bundle)
```
...
**Settings not persisting** → Cast uses Qt's native settings store. On Linux that's `~/.config/Cast/Cast.conf`. Delete to reset.
...
2. **Keep the UI clean.** Cast is a deliberately small surface. If a new feature needs a settings page, it probably doesn't belong.
...
- **[FFmpeg](https://ffmpeg.org)** — the actual conversion engine. Cast is a face for it.
